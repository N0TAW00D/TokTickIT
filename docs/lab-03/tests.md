...
| Migration / regression (M) | Vitest against a database seeded with Lab 2 data | `server/tests/lab-03/migration.test.ts`, `requester-regression.api.test.ts` |
...
Files beyond the handout §12 minimum — `password.test.ts`, `transitions.test.ts`,
`queue-query.test.ts`, `migration.test.ts`, `requester-regression.api.test.ts`, `AppShell.test.tsx`,
`ui-style.test.tsx`, `responsive.spec.ts` — are additions, not substitutions; every §12 path exists.
...
| ID | T | AC | What it tests | Expected result | File | Status |
|---|---|---|---|---|---|---|
| E2E-01 | E | AC-01, AC-05 | Login journey | Invalid login shows the safe failure; valid login enters the app showing name and role | `e2e/lab-03/authentication.spec.ts` | Planned |
| E2E-02 | E | AC-02, AC-09 | First-login password change | Normal app opens only after a valid change | `e2e/lab-03/authentication.spec.ts` | Planned |
| E2E-03 | E | AC-05 | Inactive account | Inactive login is refused with the same message as a wrong password | `e2e/lab-03/authentication.spec.ts` | Planned |
| E2E-04 | E | AC-10 | Logout | Logout returns to Login; Back and a typed protected URL both land on Login | `e2e/lab-03/authentication.spec.ts` | Planned |
| E2E-05 | E | AC-26…AC-33 | Queue to detail | Search, filter, sort and paginate, then open a ticket from the queue | `e2e/lab-03/staff-ticket-flow.spec.ts` | Planned |
...
Filled in Issue #74 from a real run on the final branch. Until then every row above reads `Planned`.

| Level | Planned | Passing | Command |
|---|---|---|---|
| Unit | 11 | _pending_ | `npm run test:server` |
| API / integration | 50 | _pending_ | `npm run test:server` |
| Security / authorization | 9 | _pending_ | `npm run test:server` |
| Migration / regression | 8 | _pending_ | `npm run test:server` |
| UI component | 18 | _pending_ | `npm run test:client` |
| UI style | 6 | _pending_ | `npm run test:client` |
| Responsive | 6 | _pending_ | `npm run test:e2e` |
| E2E | 12 | _pending_ | `npm run test:e2e` |
| **Total** | **120** | _pending_ | `npm run test:all` |

No test may be skipped, disabled or marked `.only` at release (`specification.md` §10.1).
